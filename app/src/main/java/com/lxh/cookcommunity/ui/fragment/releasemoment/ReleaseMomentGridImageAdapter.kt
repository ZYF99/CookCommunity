...
import com.lxh.cookcommunity.databinding.ItemFilterGridImageBinding
import java.util.*
...
/*			val diffResult = DiffUtil.calculateDiff(SingleBeanDiffCallBack(imgList, newList), true)
    ...
    fun onItemMove(fromPosition: Int, toPosition: Int) {
        //交换位置
        if (toPosition >= 0 && toPosition < imgList.size) {
            try {
                Collections.swap(imgList, fromPosition, toPosition)
